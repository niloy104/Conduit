...
func (h *handler) loginUser(w http.ResponseWriter, r *http.Request) {
	...
	session, err := h.server.CreateSession(h.ctx, &storer.Session{
		ID:           refreshClamis.RegisteredClaims.ID,
		UserEmail:    gu.Email,
		RefreshToken: refreshToken,
		IsRevoked:    false,
		ExpiresAt:    refreshClamis.RegisteredClaims.ExpiresAt.Time,
	})
	if err != nil {
	...
	res := LoginUserRes{
		...
		RefreshTokenExpiresAt: refreshClamis.RegisteredClaims.ExpiresAt.Time,
		User:                  toUserRes(gu),
	}
...
func (h *handler) renewAccessToken(w http.ResponseWriter, r *http.Request) {
	...
	session, err := h.server.GetSession(h.ctx, refreshClamis.RegisteredClaims.ID)
	if err != nil {
	...
	if session.UserEmail != refreshClamis.Email {
